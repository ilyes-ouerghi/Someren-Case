...
namespace Someren_Case.Repositories
{
    public class DbDrinkRepository : IDrinkRepository
    {
        ...
        public Drink GetDrinkById(int id)
        {
            ...
            {
                connection.Open();
                var command = new SqlCommand("SELECT DrinkID, Name, Type, VATRate, StockQuantity FROM Drink WHERE DrinkID = @DrinkID", connection);
        ...
        public void AddDrink(Drink drink)
        {
            ...
            {
                connection.Open();
                var command = new SqlCommand("INSERT INTO Drink (Name, Type, VATRate, StockQuantity) VALUES (@Name, @Type, @VATRate, @StockQuantity)", connection);
        ...
        public void UpdateDrink(Drink drink)
        {
            ...
            {
                connection.Open();
                var command = new SqlCommand("UPDATE Drink SET Name = @Name, Type = @Type, VATRate = @VATRate, StockQuantity = @StockQuantity WHERE DrinkID = @DrinkID", connection);
                command.Parameters.AddWithValue("@Name", drink.Name);
                command.Parameters.AddWithValue("@Type", drink.Type);
                command.Parameters.AddWithValue("@VATRate", drink.VATRate);
                command.Parameters.AddWithValue("@StockQuantity", drink.StockQuantity);
